...
public class StudyCoDAO {
	// ==========================================================================================================
	public ArrayList<StudyCoVO> SelectAll(StudyCoVO vo) {
		Connection conn = JDBC.connect();
		PreparedStatement pstmt = null;
		ArrayList<StudyCoVO> datas = new ArrayList<StudyCoVO>();
		try {
			String sql_SelectAll = "SELECT * FROM STUDYCOMMENT WHERE PNUM=? ORDER BY CNUM DESC"; // 최근 게시글 상단 배치
			pstmt = conn.prepareStatement(sql_SelectAll);
			pstmt.setInt(1, vo.getPnum());
			ResultSet rs = pstmt.executeQuery(); // executeQuery 반환타입 rs!
			while (rs.next()) {
				StudyCoVO data = new StudyCoVO();
	...
	public int getTotalCount(StudyCoVO vo) {
		Connection conn = JDBC.connect();
		PreparedStatement pstmt = null;
		int total = 0;
		try {
			conn = JDBC.connect();
			String sql = "SELECT COUNT(*) FROM STUDYCOMMENT WHERE PNUM=?";
